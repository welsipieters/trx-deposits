...
connection.connect((err) => {
    ...
    WHERE table_schema = ? AND table_name = 'sweeps' AND column_name = 'processed';
    `;
        ...
        if (results.length === 0) {
            ...
            connection.query(alterTableQuery, (alterError) => {
                if (alterError) {
                    ...
                    console.log('processed column added to sweeps table.');
                }
                connection.end();
            });
        } else {
            console.log('processed column already exists in sweeps table.');
            connection.end();
        }
    });
});
